...
Access the search bar with `Ctrl+F`. Select a search mode from the dropdown next to the search input.
...
Toggle `Ctrl+B` to show only bookmarked rows. Useful for reviewing rows you've already flagged as important.

## Cross-Tab Search

Use `Ctrl+Shift+F` to search across all open tabs simultaneously. Results show the match count per tab, letting you quickly identify which timelines contain your search term.
