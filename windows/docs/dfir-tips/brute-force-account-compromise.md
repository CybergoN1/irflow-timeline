...
Open your KAPE triage output or EVTX collection in IRFlow Timeline. Use the search bar (`Ctrl+F`) to filter to authentication events:
...
Filter to bookmarked rows only (`Ctrl+B`) for a clean attack narrative, then [export the report](/workflows/export-reports).
